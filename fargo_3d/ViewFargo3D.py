...
#output_folder = "/Users/matias/Simulations/mi_fargo3d/outputs/p3diso/"
output_folder = "/Users/matias/Simulations/mi_fargo3d/outputs/tde_3d_isoV2/"
...
mid_x =  int(int(P("NX"))/2) #corte por donde esta el planeta (en la mitad del plano azimuthal)

print(f"mid_x = {mid_x}, valor de phi en mid_x: {domain_x[mid_x]}")

# Altura z para el corte (en AU)
z_usuario = 0.0
# Encontrar el índice en domain_z más cercano al valor de z_usuario
cut_z = np.argmin(np.abs(domain_z - z_usuario))

# Angulo azimutal phi para el corte (en radianes). ej. 0 para la mitad del disco, donde normalmente está el planeta
phi_usuario = np.pi/2
# Encontrar el índice en domain_x más cercano al valor de phi_usuario
cut_x = np.argmin(np.abs(domain_x - phi_usuario))

show_planes = [["XY", 1, cut_z, snapshot1], ["YZ", 1, mid_x, snapshot2], ["YZ", 1, cut_x, snapshot3]]


print(f"Valor de theta en el mid-plane: {domain_z[mid_z]} (rad), esperado: {np.pi/2}")
