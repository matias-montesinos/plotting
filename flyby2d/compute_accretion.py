...
for output_number in output_numbers:
    ...
    time_days = time_code_units * unit_time   # Tiempo en días
...
plt.title('Luminosidad Bolométrica Total Observada $L_{\\text{bol, obs}}(t)$ con Extinción $A_V=17$ mag')
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
# Definir función para calcular luminosidad bolométrica
def calcular_luminosidad_bolometrica(mdot_r_cgs, radii_cm, M_star_cgs, L_sun_cgs):
    dLdr = (3 * G_cgs * M_star_cgs * mdot_r_cgs) / (2 * radii_cm**2)  # erg/s/cm
    Lbol = np.trapz(dLdr, radii_cm)  # erg/s
    Lbol_solar = Lbol / L_sun_cgs  # L_sun
    return Lbol_solar

# Definir función para aplicar extinción
def calcular_luminosidad_observada(Lbol, A_v):
    return Lbol * 10**(-0.4 * A_v)

# Parámetros
A_V = 17.0  # magnitudes
M_star = 1.0  # Masa en M_sun
M_star_cgs = M_star * M_sun_cgs

# Convert radii to cm for calculations
radii_cm = radii * unit_length  # cm

# Initialize a matrix to store Lbol(r, t) in solar units
# Dimensions: time x radius
Lbol_matrix = np.zeros((len(time_array), num_radii))  # [L_sun]

# Efficiency factor for converting accretion rate to luminosity
eta = 0.1  # Adjust based on your model's assumptions
...
for i, output_number in enumerate(output_numbers):
    ...
    for j, r0 in enumerate(radii):
        ...
        # Convert \dot{M} from Msun/yr to g/s
        mdot_cgs = mdot * 1.989e33 / 3.154e7  # [g/s]
        
        # Calculate Lbol at this r0, t using the formula:
        # Lbol = eta * G * M_star * \dot{M} / r
        Lbol = eta * G_cgs * M_star_cgs * mdot_cgs / radii_cm[j]  # [erg/s]
        
        # Convert Lbol to solar units
        Lbol_solar = Lbol / L_sun_cgs  # [L_sun]
        
        # Apply extinction to obtain observed luminosity
        Lbol_obs = calcular_luminosidad_observada(Lbol_solar, A_V)  # [L_sun]
        
        # Store the observed luminosity in the matrix
        Lbol_matrix[i, j] = Lbol_obs

# Generate the plot of Lbol(r, t) as a heatmap
# Create a meshgrid of radii and times for plotting
R, T = np.meshgrid(radii, time_array)  # R in [AU], T in [days]
...
# Use pcolormesh for the heatmap
heatmap = plt.pcolormesh(R, T, Lbol_matrix, shading='gouraud', cmap='plasma')  # You can choose another cmap if preferred

# Add a color bar with label in English
cbar = plt.colorbar(heatmap)
cbar.set_label('Bolometric Luminosity [$L_\\odot$]')  # Color bar label in English

# Set labels and title in English
plt.xlabel('Radius [AU]')
plt.ylabel('Time [days]')
plt.title('Bolometric Luminosity $L_{\\text{bol}}(r, t)$')
